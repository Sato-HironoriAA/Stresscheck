...
class ResultInfoPdfCreator
{
    ...
    */
    ...
    private function pdf_init()
    {
        ...
        // UTF-16フォントロード
        $this->pdf->AddUniJISFont('MS-Mincho','UniJIS_Mincho');
        $this->pdf->AddUniJISFont('MS-Gothic','UniJIS_Gothic');

        // 文字書き込み：明朝体
        $this->pdf->SetFont('UniJIS_Mincho', '', 12);
    ...
    private function pdf_create_person1($data, $resultList, $procNendo, $mode)
    {
        ...
        $this->pdf->SetFont('UniJIS_Mincho', '', 11);
        $this->pdf->SetTextColor(0,0,0);
        ...
        $this->pdf->SetFont('UniJIS_Mincho', '', 10);
        $margin = 3.3;
...
/*
            ...
            if ($this->dba->getCount($result) > 0)
            {
                $data2 = $this->dba->getFetchArray($result);

                if (!is_null($data2['CHECK_RESULT_LOB']))
                {
                    $resultList = unserialize($data2['CHECK_RESULT_LOB']);
                }
            }
            ...
            if (isset($resultList))
            {
                ...
                $pos = 0;
                while ($data2 = $this->dba->getFetchArray($result))
                {
                    $groupList[$pos][0] = $data2['CHECK_ITEM_GROUP_CD'];
                    $groupList[$pos][1] = $data2['CHECK_ITEM_GROUP_STR'];
                    $pos++;
                }
                ...
                $aj = new AutoJudge();
                $stressState2 = $aj->getJudge($resultA, $resultB, $resultC, $data['SEX'], $data['COMPANY_CD']);
            }
*/
        ...
        $this->pdf->SetFont('UniJIS_Mincho', '', 16);
        $this->pdf->SetTextColor(0,0,0);
        ...
        //$endPosRow = $this->pdf_text($globalStr["2"], $endPosRow, $rowWidth, $margin);

//$doctorStressState=$jm->getValue('stress', $this->stress_state_change(3));
//$doctorStressReaction=$jm->getValue('stress_reaction', "000001");
//$doctorExplain1=$jm->getValue('stress_info', $this->stress_state_change(4));
//$doctorStresser=$jm->getValue('stresser', "11111");
//$doctorExplain3=$jm->getValue('stress_info3', $this->stress_state_change(4));

        $endPosRow = $this->pdf_text("", $endPosRow + $rowHeight, $rowWidth, $margin);
        $this->pdf->SetFont('UniJIS_Mincho', 'B', 12);
        ...
        //$endPosRow = $this->pdf_text("", $endPosRow + 4, $rowWidth, $margin);
        //$endPosRow = $this->pdf_text("【ストレスによっておこる心身の反応】", $endPosRow + $rowHeight, $rowWidth - 4, $margin);
        //$endPosRow = $this->pdf_text($doctorStressReaction, $endPosRow + $rowHeight, $rowWidth, $margin);
        $endPosRow = $this->pdf_text("", $endPosRow + $rowHeight, $rowWidth, $margin);
        $this->pdf->SetFont('UniJIS_Mincho', 'B', 12);
        $endPosRow = $this->pdf_text("【ストレスの原因と考えられる因子】", $endPosRow + $rowHeight, $rowWidth - 3, $margin);
        $this->pdf->SetFont('UniJIS_Mincho', '', 12);
        //$endPosRow = $this->pdf_text($doctorExplain2, $endPosRow + $rowHeight, $rowWidth, $margin);
    ...
    private function pdf_create_person2($data, $resultList, $procNendo, $mode)
    {
        ...
        $this->pdf->SetFont('UniJIS_Gothic', '', 12);
        $this->pdf_text($data['NAME'], 43.5, 23, 4);
        
        $this->pdf->SetFont('UniJIS_Gothic', '', 20);
        $str = "○";
        ...
        /*$this->pdf_text($ajRetEvaluationTotal, 269.2, 129, 4);
        $this->pdf_text("19", 269.2, 155.8, 4);
        $maxTotal = "92";
        if ($data['SEX'] == 2)
        {
            $maxTotal = "93";
        }
        $this->pdf_text($maxTotal, 269.2, 173, 4);*/
    }
    ...
    private function pdf_text($str, $row, $col, $margin)
    {
        $posRow = $row;
        $posCol = $col;
        $pattern = '/^(?:\xEF\xBD[\xA1-\xBF]|\xEF\xBE[\x80-\x9F]|[a-zA-Z0-9]|[0-9]|[ -\/:-@\[-`\{-\~]|\s)+$/D';

        $lf = "\n";
        for ($i=0; $i<mb_strlen($str); $i++)
        {
            $strChar = mb_substr($str, $i, 1, 'utf-8');
            if ($strChar == $lf)
            {
                $posRow += 5.6;
                $posCol = $col;
            ...
            {
                $writeStr = mb_convert_encoding($strChar, "unicode", "utf-8");
                $this->pdf->Text($posCol, $posRow, $writeStr);
                if(preg_match($pattern, $strChar))
                {
                    $posCol += ($margin - 2);
                }
                else
    ...
    private function pdf_create_examinee_list($result, $examineeCnt, $examineeNoCnt, $examineeExecCnt, $examineeNotCnt, $consentOkCnt, $examineeDate)
    {
        ...
        while ($data = $this->dba->getFetchArray($result))
        {
            ...
            if (!$titleFlg)
            {
                ...
                $this->pdf->SetFont('UniJIS_Mincho', '', 9);
                $this->pdf->SetTextColor(0,0,0);
    ...
    {
        $this->pdf->SetFont('UniJIS_Mincho', '', $size);
        $this->pdf->SetTextColor(0,0,0);
    }
